...
from models import db_models, business_models
...
@router.get("/users", response_model=list[business_models.UserProfileOut])
def list_users(
...
@router.post("/users", response_model=business_models.UserProfileOut)
def provision_user(
    req: business_models.UserProvisioningRequest,
    admin: db_models.IdentityUser = Depends(require_admin),
    session: Session = Depends(db.get_session)
):
...
@router.patch("/users/{user_id}", response_model=business_models.UserProfileOut)
def update_user_profile(
    user_id: int,
    payload: business_models.UserProfileUpdate,
    admin: db_models.IdentityUser = Depends(require_admin),
    session: Session = Depends(db.get_session)
):
